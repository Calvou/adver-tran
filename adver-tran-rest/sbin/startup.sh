...
APPLICATION_MAIN=com.adver.tran.AdverApplication
...
#JAVA_OPTS="-Dspring.profiles.active=$EVN -Dserver.port=$PORT -Duser.timezone=GMT+8 -server -Xms1024m -Xmx4096m -Xloggc:${LOG_DIR}/gc.log"
JAVA_OPTS="-Dspring.profiles.active=$EVN -Duser.timezone=GMT+8 -server -Xms1024m -Xmx2048m -Xloggc:${LOG_DIR}/gc.log"
...
getPID(){
    javaps=`jps -vl | grep $EVN | grep $APPLICATION_MAIN`
    if [ -n "$javaps" ]; then
